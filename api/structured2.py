...
class StructuredReasoner(Reasoner):
    ...
    def extract_info(self, info_format, output_type: Union[BaseModel, Type]):
        ...
        use_pydantic = type(output_type) is type and issubclass(
        ...
        response = chatgpt.complete(messages=self.messages, model=self.model, functions=[
                                    json_schema], function_call={'name': func_name}, use_cache=True)
        if response['role'] != 'function':
            raise Exception(
                f"Expected a function call, but got: {response['content']}")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if THINK_FIRST:
            thought = reasoner.internal_monologue(
                "I should brainstorm a list of funny ways to respond.")
            printc('\n' + thought, color='blue')
        else:
            reasoner.add_message(
                'assistant', '[Internal Monologue]: I should brainstorm a list of funny ways to respond.')
        options = reasoner.extract_info(
            "I came up with the following options:\n{options}", List[str])
        printc('\nOptions:\n- ' + '\n- '.join(options), color='yellow')

        if THINK_FIRST:
            thought = reasoner.internal_monologue(
                "I need to choose the funniest response, I can only choose one. My options are:\n" + '\n'.join(options))
            printc('\n' + thought, color='blue')
        else:
            numbered_options = "\n".join(
                [f"{i+1}. {option}" for i, option in enumerate(options)])
            reasoner.add_message(
                'assistant', '[Internal Monologue]: I need to choose the funniest response. My options are:\n' + numbered_options)
        choice = reasoner.extract_info("I chose Option {choice_index}.", int)
        printc('\nChose response: ' + options[choice-1], color='yellow')

        response = reasoner.external_dialogue(
            f"I'll respond to the user using the response I chose.")
        print('\n' + response)
